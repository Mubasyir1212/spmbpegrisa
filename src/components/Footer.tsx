    ...
    <footer className="bg-slate-900 text-slate-300 py-12 border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="col-span-1 md:col-span-2">
            <Link to="/" className="flex items-center gap-2 mb-4">
              ...
              <span className="font-bold text-xl tracking-tight text-white">
                {settings?.namaSekolah || 'SMP PGRI 1 Sempor'}
              </span>
            </Link>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-3">
                ...
                <span>{settings?.alamat || 'Jl. Sempor Baru, Jatinegara, Sempor - Kebumen'}</span>
              </li>
              <li className="flex items-center gap-3">
                <Phone size={18} className="text-blue-500 shrink-0" />
                <span>{settings?.telepon || '+62882-2775-1500'}</span>
              </li>
              <li className="flex items-center gap-3">
                <Mail size={18} className="text-blue-500 shrink-0" />
                <span>{settings?.email || 'smppgri1sempor@gmail.com'}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-12 pt-8 border-t border-slate-800 text-center text-sm text-slate-500 flex flex-col md:flex-row justify-between items-center">
          <p>&copy; {new Date().getFullYear()} {settings?.namaSekolah || 'SMP PGRI 1 Sempor'}. Hak Cipta Dilindungi.</p>
          <p className="mt-2 md:mt-0">Pegrisa SPMB Online Terintegrasi</p>
        </div>
      </div>
    </footer>
  );
